...
export default function GeneratePage() {
  ...
  const fetchPOIs = async () => {
    ...
    // Much larger radius based on pace and duration
    const paceMultiplier = pace === 'slow' ? 40 : pace === 'fast' ? 80 : 60
    const radius = Math.min(duration * paceMultiplier, 10000) // Up to 10km radius
    ...
    try {
      ...
      if (vibe === 'quiet') {
        ...
        if (fetchedPOIs.length < 5) {
          // If too few quiet places, just take closest ones
